...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div>
            ...
            <textarea name="mini_desc" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Brief description for project preview">{{ old('mini_desc', $project->mini_desc) }}</textarea>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Full Description</label>
            <textarea name="full_desc" rows="6" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Detailed project description">{{ old('full_desc', $project->full_desc) }}</textarea>
        </div>
        ...
        <div>
            ...
            <img src="{{ resolve_project_image($project) }}" alt="Current card thumbnail" class="w-40 h-30 object-cover rounded-lg mb-2 border" onerror="this.style.display='none'">
