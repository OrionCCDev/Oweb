...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.clients.update', $client) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $client->name) }}" required class="w-full px-4 py-2 border rounded-lg">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Description</label>
            <textarea name="description" rows="4" class="w-full px-4 py-2 border rounded-lg">{{ old('description', $client->description) }}</textarea>
        </div>
        <div>
        ...
        <div>
            ...
            @if (resolve_client_logo($client))
                <img src="{{ resolve_client_logo($client) }}" alt="{{ $client->name }}" class="w-32 h-20 object-contain mb-2 border rounded p-2 bg-gray-50">
